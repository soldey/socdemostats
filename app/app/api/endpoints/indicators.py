    ...
    for indicator in indicators:
        ...
        response.append(
            schemas.IndicatorShortDescriptionResponse(
                id=indicator.id,
                name=indicator.name,
                unit_name=indicator.unit.unit_name
            )
        )
    return response
...
async def create_indicator(indicator: schemas.IndicatorCreateRequest, db: AsyncSession = Depends(get_db)):
    ...
    unit = await indicator.awaitable_attrs.unit
    response = schemas.IndicatorShortDescriptionResponse(
        id=indicator.id,
        name=indicator.name,
        unit=unit.name
    )
...
async def read_indicator_details(indicator_id: int, db: AsyncSession = Depends(get_db)):
    ...
    aggregated_values_availability = await indicator_crud.get_aggregated_indicator_values_availability(db, indicator_id=indicator_id)
    aggregated_availability_data = [schemas.IndicatorAvailability(**datum) for datum in aggregated_values_availability] 
    detailed_values_availability = await indicator_crud.get_detailed_indicator_values_availability(db, indicator_id=indicator_id)
...
async def read_aggregated_indicator_values(indicator_id:int, territory_id: int, db: AsyncSession = Depends(get_db)):
    indicators = await indicator_crud.get_aggregated_indicator_values(db, indicator_id=indicator_id, territory_id=territory_id)
    response = []
    for i in indicators:
        unit = await i.indicator.awaitable_attrs.unit
        response.append(schemas.IndicatorAggregatedResponse(
            id=i.id,
...
async def load_aggregated_indicator_values(indicator_id:int, territory_id: int, indicator_values: list[schemas.LoadIndicatorAggregatedRequest], db: AsyncSession = Depends(get_db)):
    ...
    # if not indicator:
    #     raise HTTPException(status_code=404, detail=f"Indicator with id {indicator_id} not found")
    indicator_values = await indicator_crud.create_aggregated_indicator_values(db=db, indicator_id=indicator_id, territory_id=territory_id, indicator_values=indicator_values)
